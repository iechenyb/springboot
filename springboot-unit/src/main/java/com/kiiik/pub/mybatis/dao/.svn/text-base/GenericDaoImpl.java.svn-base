...
public class GenericDaoImpl implements GenericDao {
	...
	@Override
	public int deleteDBEntityByDate(Object entity, Object start, Object end) {
		EntityInfo info = getInfoWithValue(entity);
		if (!info.hasKeyCol()) {
			throw new IllegalArgumentException("update cannot done when key property is null");
		}
		int rows = genericDao.deleteDBEntityByDate(info,start,end);
		return rows;
	}
	
	//全表删除记录 add by qinxiang in 2019-11-15
	@Override
	public int deleteFullDBEntity(Object entity) {
		EntityInfo info = getInfoWithValue(entity);
		if (!info.hasKeyCol()) {
			throw new IllegalArgumentException("update cannot done when key property is null");
		}
		int rows = genericDao.deleteFullDBEntity(info);
		return rows;
	}
